...
class RsaUtil(object):
    ...
    def __init__(self,
        ...
        if company_pub_file:
            with open(company_pub_file, 'rb') as f:
                self.company_public_key = rsa.PublicKey.load_pkcs1(f.read())
                print("----------------company_public_key--------------------")
                print(self.company_public_key )
        if company_pri_file:
            with open(company_pri_file, 'rb') as f:
                self.company_private_key = rsa.PrivateKey.load_pkcs1(f.read())
                print("----------------company_private_key--------------------")
                print(self.company_private_key)
    ...
    def sign_by_private_key(self, data):
        """私钥签名.
            ...
            签名之后，需要转义后输出
        """
        signature = rsa.sign(data, self.company_private_key, 'SHA-1')
...
message = 'hhhjk<.09872332qwe'.encode('utf8')
print("明文内容：>>> ")
print(message)
rsaUtil = RsaUtil()
